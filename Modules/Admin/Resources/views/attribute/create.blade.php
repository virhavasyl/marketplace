...
@section('content')
  <div class="animated fadeIn">
    <div class="row justify-content-center">
      <div class="col-sm-8">
        <div class="card">
          <form method="POST" action="{{ LaravelLocalization::getLocalizedURL(null, '/attributes') }}">
            @csrf
            <div class="card-header">
              <strong>@lang('admin::attribute.form.header.create')</strong>
            </div>
            <div class="card-body">
              <div class="row">
                <div class="col-sm-8">
                  <div class="form-group">
                    ...
                    <select id="category_id" name="category_id[]" class="select2 form-control w-100 {{ $errors->has('category_id') ? ' is-invalid' : '' }}" multiple="multiple">
                      @foreach($categories as $category)
                        <option class="l1" value="{{ $category->id }}" {{ in_array($category->id, old('category_id', [])) ? 'selected' : '' }}>{{ $category->title }}</option>
                        @foreach($category->children as $subcategory)
                          <option class="l2" value="{{ $subcategory->id }}" {{ in_array($subcategory->id, old('category_id', [])) ? 'selected' : '' }}>{{ $subcategory->title }}</option>
                          @foreach($subcategory->children as $subsubcategory)
                            <option class="l3" value="{{ $subsubcategory->id }}" {{ in_array($subsubcategory->id, old('category_id', [])) ? 'selected' : '' }}>{{ $subsubcategory->title }}</option>
                          @endforeach
                        @endforeach
                      @endforeach;
                    </select>
              ...
              <!-- /.row -->
              <table class="table table-responsive-sm table-bordered variation-table" {{ old('type') != $list_type ? 'style=display:none' : '' }}>
                <thead>
                  <tr>
                    @foreach($locales as $locale)
