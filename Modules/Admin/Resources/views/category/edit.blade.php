...
@section('content')
  <div class="animated fadeIn">
    <div class="row justify-content-center">
      <div class="col-sm-12">
        <div class="card">
          <form method="POST" action="{{ Helper::generateTranslatableUrl("/categories/{$model->id}", "parent_id") }}">
            @method('PUT')
            @csrf
            <div class="card-header">
              <strong>@lang('admin::category.form.header.edit')</strong>
            </div>
            <div class="card-body">
              <div class="row">
                <div class="col-sm-4">
                  <div class="form-group">
                    ...
                    <select id="parent_id" name="parent_id" class="select2 form-control {{ $errors->has('parent_id') ? ' is-invalid' : '' }}">
                      ...
                      @foreach($categories as $category)
                        <option class="l1" value="{{ $category->id }}" {{ old('parent_id', $model->parent_id) == $category->id ? 'selected' : '' }}>{{ $category->title }}</option>
                        @foreach($category->children as $subcategory)
                          <option class="l2" value="{{ $subcategory->id }}" {{ old('parent_id', $model->parent_id) == $subcategory->id ? 'selected' : '' }}>{{ $subcategory->title }} ({{ $category->title }})</option>
                        @endforeach
                      @endforeach;
                    </select>
                  ...
                  <div class="col-sm-4">
                    <div class="form-group">
                      ...
                      <textarea class="form-control"
                                ...
                                rows="5">{{ old("description:$locale", $model->{"description:$locale"}) }}</textarea>
