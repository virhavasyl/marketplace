...
@section('content')
  <div class="animated fadeIn">
    <div class="row justify-content-center">
      <div class="col-sm-8">
        <div class="card">
          <form method="POST" action="{{ Helper::generateTranslatableUrl("/locations/{$model->id}", "parent_id") }}">
            @method('PUT')
            @csrf
            <div class="card-header">
              <strong>@lang('admin::location.form.header.edit')</strong>
            </div>
            <div class="card-body">
              <div class="row">
                <div class="col-sm-6">
                  <div class="form-group">
                    ...
                    <select id="parent_id" name="parent_id" class="select2 form-control {{ $errors->has('parent_id') ? ' is-invalid' : '' }}" data-target="type">
                      ...
                      @foreach($locations as $region)
                        <option class="l1" value="{{ $region->id }}" data-type="2" {{ old('parent_id', $model->parent_id) == $region->id ? 'selected' : '' }}>{{ $region->title }}</option>
                        @foreach($region->children as $district)
                          <option class="l2" value="{{ $district->id }}" data-type="3" {{ old('parent_id', $model->parent_id) == $district->id ? 'selected' : '' }}>{{ $district->title }} ({{ $region->title }})</option>
                        @endforeach
                      @endforeach;
                    </select>
                  ...
                  <div class="form-group">
                    ...
                    <select id="type" class="form-control type" disabled>
                      @foreach($types as $key => $value)
                        <option value="{{ $key }}" {{ old('type', $model->type) == $key ? 'selected' : '' }}>{{ $value }}</option>
                      @endforeach
                    </select>
                    <input class="type" type="hidden" name="type" value="{{ old('type', $model->type) }}" />
                    @if ($errors->has('type'))
                      <div class="invalid-feedback" role="alert">
                        {{ $errors->first('type') }}
                      </div>
