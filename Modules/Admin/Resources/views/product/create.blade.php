...
@section('content')
  <div class="animated fadeIn">
    <div class="row justify-content-center">
      <div class="col-md-6">
        <div class="card">
          <form method="POST" action="{{ LaravelLocalization::getLocalizedURL(null, '/products') }}">
            @csrf
            <div class="card-header">
              <strong>@lang('admin::product.form.header.create')</strong>
            </div>
            <div class="card-body">
              <div class="row">
                <div class="col-sm-12">
                  ...
                  <div class="form-group {{ $errors->has('category_id') ? 'is-invalid-group' : '' }}">
                    ...
                    <div class="input-group">
                      <input id="category_text"
                             ...
                             value="{{ old('category_id') && isset($categories['level3'][old('category_id')]) ? $categories['level3'][old('category_id')]['title'] : '' }}"
                             readonly />
                      ...
                      <div class="col-2">
                        <select id="currency" name="currency_id" class="form-control w-100 {{ $errors->has('currency_id') ? ' is-invalid' : '' }}">
                          @foreach($currencies as $currency)
                            <option value="{{ $currency->id }}" {{ old('currency_id') == $currency->id ? 'selected' : '' }}>{{ $currency->iso_code }}</option>
                          @endforeach;
                        </select>
                  ...
                  <!-- /.form-group -->
                  <div class="form-group">
                    <input id="status" type="checkbox" name="status" value="{{ $status_active }}" checked />
